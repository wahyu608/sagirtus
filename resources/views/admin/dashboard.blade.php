            ...
            function tampilkanMarker() {
                ...
                desaData.forEach(desa => { //menampilkan
                    if (selectedDesa === "all" || desa.id == selectedDesa) { // jika semua desa atau desa yang dipilih  
                        let seniRitusCount = seniRitusData.filter(item => item.desa_id == desa.id).length;  
                        let setView = map.setView([desa.latitude, desa.longitude], 10); //menghitung jumlah seni & ritus
                        ...
                        let popupContent = `
                            ...
                            <a href="/admin/data?desa=${desa.id}&jenis=""">Lihat Data</a>
                        `; //konten popup
